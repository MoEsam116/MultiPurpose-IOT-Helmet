 ...
 *==============================================================================================================================================*/
uint16_t MQ5_u16GetRatio(void)
{
	/* variable to store the result */
	uint16_t RES[100] = {0};

	/* variable to store RS Air & Ratio */
	uint16_t RS_Air=0, Ratio=0;

	uint32_t sum=0;
	/* variable to store RS value */
	float RS = 0;

	/* variable used as iterator */
	uint8_t Iterator = 0;

	/* loop to get the average of 100 samples */
	for (Iterator = 0; Iterator < 100; Iterator++)
	{
		ADC_u8StartSingleConv(&ADC_Configs, (uint16_t *)&RES[Iterator]);
		sum += RES[Iterator];
	...
	/* calculate RS value by dividing the sum of 100 samples by 100 */
	RS = sum / 100;

	/* change the RS value from digital to analog by multiplying it by 3.3 and dividing it by 4096 */
	RS = (RS * (float)3.3) / 4096;

	/* Calculate RS Air */
	RS_Air = ((float)3.3 - RS) / RS;
